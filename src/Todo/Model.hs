...
-- import qualified Data.List.Unique   as List.Unique
...
deleteTag tag db@TodoDB { .. } = 
    case List.find (\todo -> todoTags todo == tag :| []) (getByTag tag tododbTodos) 
    of Nothing -> TodoDB { tododbTodos = map (\Todo{ .. } -> Todo { todoTags = fromList $ (List.delete tag) . toList $ todoTags, .. }) tododbTodos
                         , tododbTags  = List.delete tag tododbTags }
       Just _ -> db

editTag :: Tag -> Tag -> TodoDB -> TodoDB
editTag old new TodoDB{ .. } = 
    TodoDB { tododbTodos = map (\Todo{ .. } -> Todo { todoTags = fromList $ replace $ toList todoTags, .. }) tododbTodos
           , tododbTags  = new : List.delete old tododbTags } where 
            replace :: [Tag] -> [Tag]
            replace [] = []
            replace (t:ts) = if t == old then new : ts else t : replace ts
